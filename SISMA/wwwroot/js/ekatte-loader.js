var ekatteUrls = {
    ekatteSearch: rootDir + 'Ajax/SearchEkatte?query=',
    ...
    rajonGet: rootDir + 'Ajax/GetDDL_EkRajon',
    vidAddressGet: rootDir + 'Ajax/GetDDL_VidAddress'
};

let ekkateObj = (function () {
    let _vidAddress = [];
    const vidAddress = () => _vidAddress;
    const fetchData = function () {
        return new Promise(function (resolve, reject) {
            if (_vidAddress.length > 0) {
                resolve(true);
            }
            requestGET_Json(ekatteUrls.vidAddressGet, {}, function (items) {
                _vidAddress = items;
                resolve(true);
            });
        });
...
function initEkatte() {
    $('.ekatte-container').each(function (i, e) {
        let ekatteControl = $(e).find('.ekatte-control:first');
        if ($(ekatteControl).data('isloaded') === 'true') {
            return;
        }
        $(ekatteControl).data('isloaded', 'true');
            ...
            _renderMenu: function _renderMenu(ul, items) {
                ...
                $.each(items, function (index, item) {
                    if (item.category !== category) {
                        ul.append('<li class="ui-autocomplete-category ui-state-disabled" aria-label="' + item.category + '" value="-1">' + item.category + '</li>');
        ...
        $(ekatteControl).autocomplete_custom({
            ...
            select: function select(event, ui) {
                if (ui.item) {
                    ...
                    var input_hidden = $(e).find('.ekatte-val');
                    ...
                    if (input_hidden) {
                        ...
                        //let rajonControl = $(e).find('.rajon-ekatte');
                        //requestCombo(ekatteUrls.rajonGet, { eknm: ui.item.id }, rajonControl, $(rajonControl).data('id'), null);
                    }
                }
            }
        }).change(function () {
            ...
            if (!input.value || input.value < 2) {
                var input_hidden = input.parentElement.find('input[type="hidden"]');
                input_hidden.value = '';
            }
        }).blur(function () {
            var input = this;
        ...
        if (ekatteVal) {
            loadEkatte(ekatteControl, ekatteVal, function () {
                //let addressContainer = $(ekatteControl).parents('.address--control:first');
                //let rajonControl = $(addressContainer).find('.rajon-ekatte');
                //requestCombo(ekatteUrls.rajonGet, { eknm: ekatteVal }, rajonControl, $(rajonControl).data('id'), null);
            });
        }
    });
